...
#[test]
fn find_text_skips_binary_files() {
    let root = temp_tree("find-text-bin");
    fs::write(root.path.join("text.txt"), "hello there").expect("failed to write text file");
    fs::write(root.path.join("blob.bin"), b"hello\x00world").expect("failed to write binary file");
    ...
    assert!(stdout.contains("text.txt"), "stdout was: {stdout}");
    assert!(!stdout.contains("blob.bin"), "stdout was: {stdout}");
    assert!(
        stderr.contains("Skipped 1 binary file"),
        "stderr was: {stderr}"
    );
}

#[test]
fn find_regex_matches_multiple_patterns_and_lines() {
    let root = temp_tree("find-regex");
    fs::write(
        root.path.join("code.rs"),
        "fn main() {}\n// TODO: refactor\n// FIXME: cleanup\n",
    )
    .expect("failed to write file");
    fs::write(root.path.join("notes.md"), "nothing here").expect("failed to write file");
    ...
    assert!(stdout.contains("code.rs"), "stdout was: {stdout}");
    assert!(!stdout.contains("notes.md"), "stdout was: {stdout}");
    assert!(
        stdout.contains("2 match(es) in 1 file(s)"),
        "stdout was: {stdout}"
    );
}

#[test]
fn find_regex_invalid_pattern_returns_clean_error() {
    let output = oak()
        .args(["--no-color", "--no-icons", "--find-regex", "("])
        .output()
        .expect("failed to run oak");
    let stderr = String::from_utf8_lossy(&output.stderr);
